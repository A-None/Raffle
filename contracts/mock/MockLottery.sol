...
import "../interfaces/IMockRandom.sol";
...
// 4 numbers
contract MockLottery is LotteryOwnable, Initializable {
    ...
    function setRandomNumber(uint8[4] memory _randomness) external onlyRandom {
        drawing(_randomness);
    }

    function startDrawing(uint8[4] memory _externalRandomNumber) external onlyAdmin {
    ...
    function drawing(uint8[4] memory _externalRandomNumber) internal {
        require(!drawed(), "please call reset function first");
        require(drawingPhase, "enter drawing phase first");

        winningNumbers = _externalRandomNumber;
        historyNumbers[issueIndex] = winningNumbers;
        historyAmount[issueIndex] = calculateMatchingRewardAmount();
        console.log(historyAmount[issueIndex][0]);
        console.log(historyAmount[issueIndex][1]);
        console.log(historyAmount[issueIndex][2]);
        console.log(historyAmount[issueIndex][3]);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            ...
            uint256 tokenId = lotteryNFT.newLotteryItem(msg.sender, _numbers[i], minPrice[0], issueIndex, block.timestamp);
            lotteryInfo[issueIndex].push(tokenId);
            if (userInfo[msg.sender].length == 0) {
                totalAddresses = totalAddresses + 1;
                userNumTickets[msg.sender] = 0;
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        ...
        require(_winningNumbers[0] != maxNumber, "not drawed");
